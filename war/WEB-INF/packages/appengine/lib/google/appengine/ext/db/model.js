...
Model.extend = function(kind, properties) {
    ...
    ctor._properties = Hash.merge(this.properties(), _initializeProperties(ctor, properties));
...
Model.updateProperties = function(newProperties) {
    Hash.update(this._properties, _initializeProperties(this, newProperties));    
}
 ...
 *   constructor data.
 */
Model.getOrInsert = function(name, data) {	
    var obj, 
        self = this;

    db.runInTransaction(function() {
        obj = self.getByKeyName(name, data.parent);
        if (!obj) {
            obj = new self(data);
            obj.put();
        }
